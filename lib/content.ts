export const weddingContent = {
  couple: {
    bride: "Marina",
    groom: "Bruno",
    fullNames: "Marina & Bruno",
  },
  date: {
    display: "26 de setembro de 2026",
    weekday: "Sábado",
    time: "18h30",
  },
  cover: {
    subtitle: "Moara",
  },
  locations: {
    ceremony: {
      title: "Cerimônia",
      name: "Igreja Sagrado Coração de Jesus",
      address:
        "Av. Francisco Barreto Leme, 730 - Vila Sao Geraldo, Taubaté",
      mapUrl:
        "https://maps.google.com/maps?vet=10CAAQoqAOahcKEwig_a2mhMyVAxUAAAAAHQAAAAAQDw..i&aep=1&sca_esv=b640ccf014533559&pvq=CgsvZy8xdGhnajQ5OSInCiFpZ3JlamEgc2FncmFkbyBjb3Jhw6fDo28gZGUgamVzdXMQAhgD&lqi=CilpZ3JlamEgc2FncmFkbyBjb3Jhw6fDo28gZGUgamVzdXMgdGF1YmF0ZUipoqD35YCAgAhaQxAAEAEQAhADEAQYARgCGAMYBBgFIilpZ3JlamEgc2FncmFkbyBjb3Jhw6fDo28gZGUgamVzdXMgdGF1YmF0ZTICcHSSAQ9jYXRob2xpY19jaHVyY2iaASNDaFpEU1VoTk1HOW5TMFZKUTBGblNVTjRNWFF5Tm1OQkVBRfoBBAgAECI&fvr=1&cs=1&um=1&ie=UTF-8&fb=1&gl=br&sa=X&ftid=0x94ccf9bb0f3f56cb:0xe138231979384a11",
      image: "gallery/igreja.webp",
      schedule: [
        {
          time: "18h30",
          title: "Cerimônia",
          description:
            "Celebraremos nossa união na Igreja Sagrado Coração de Jesus.",
        },
      ],
    },
    party: {
      title: "Festa",
      name: "Moara",
      address:
        "km 12, Rod. Floriano Rodrigues Pinheiro - Jardim Maracaibo, Tremembé",
      mapUrl:
        "https://www.google.com/maps?vet=10CAAQoqAOahcKEwi4o9fQ-cuVAxUAAAAAHQAAAAAQCA..i&pvq=Cg0vZy8xMWM2ejEza3gwIgsKBW1vYXJhEAIYAw&lqi=CgVtb2FyYUiAj8fgm6uAgAhaDxAAGAAiBW1vYXJhMgJwdJIBDndlZGRpbmdfYnVmZmV0&fvr=1&cs=1&um=1&ie=UTF-8&fb=1&gl=br&sa=X&ftid=0x94cc58e412af2303:0x94b769704091654c",
      image: "gallery/moara.jpg",
      schedule: [
        {
          time: "21h00",
          title: "Jantar e festa",
          description: "Música, dança e muita alegria.",
        },
      ],
    },
  },
  dressCode: "Traje social elegante",
  story: {
    title: "Nossa história",
    paragraphs: [
      "Nos conhecemos em um dia de sol e café, e desde então cada capítulo da nossa história foi escrito com carinho, risadas e muito amor.",
      "Agora convidamos você a celebrar conosco o início de uma nova etapa — e queremos que faça parte desse momento especial.",
    ],
  },
  photos: {
    hero: "gallery/Bruno%20&%20Marina%20-%20Pre%20Wedding-182.jpg",
    gallery: ["gallery/Bruno & Marina - Pre Wedding-110.jpg", "gallery/Bruno & Marina - Pre Wedding-116.jpg"],
  },
  messages: {
    phoneNotFound:
      "Não encontramos este número. Verifique ou contacte os noivos.",
    rsvpSuccess: "Presença confirmada com sucesso!",
    rsvpDialogIntro:
      "Seu convite é válido para as pessoas abaixo. Confirme quem estará presente na celebração.",
    giftsTitle: "Lista de presentes",
    giftsSubtitle:
      "Sua presença é o nosso maior presente. Se desejar, escolha um item abaixo.",
    cartBarCta: "Ver carrinho",
    cartSheetTitle: "Seu carrinho",
    cartEmpty: "Seu carrinho está vazio.",
    cartTotal: "Total",
    cartPay: "Pagar",
    cartPaying: "Redirecionando...",
    cartPaymentCard: "Cartão de crédito",
    cartPaymentPix: "Pix",
    cartPixCta: "Gerar código Pix",
    cartCheckoutError: "Não foi possível iniciar o pagamento. Tente novamente.",
    pixEyebrow: "Pagamento",
    pixTitle: "Pague com Pix",
    pixDescription:
      "Copie o código abaixo e cole no app do seu banco. O valor já está incluído no código.",
    pixCodeLabel: "Pix copia e cola",
    pixCopyCta: "Copiar código Pix",
    pixCopied: "Código copiado!",
    pixCopyError: "Não foi possível copiar automaticamente. Selecione e copie o código manualmente.",
    pixConfirmCta: "Já realizei o pagamento",
    pixInvalidLink: "Não encontramos os dados do Pix. Volte ao carrinho e tente novamente.",
    pixSuccessDescription:
      "Obrigado pelo carinho! Assim que o Pix for identificado, seu presente será confirmado.",
  },
} as const;
